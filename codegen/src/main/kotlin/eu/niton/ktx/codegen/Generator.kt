    ...
    fun generate() {
        ...
        files.forEach { (packageName, files) ->
            files.forEach { (fileName, contents) ->
                ...
                target.writeFile(fileName, packageName) {
...
private fun Generator.getTag(tag: TagInfo): TagFile {
    return tags.nonConcurrentComputeIfAbsent(tag.name) {
        ...
        val contentType = getContentType(tag.content, tag.name + "TagContent", filename = className)
        val contentTypeAlias = contentType?.let {
            TypeAliasSpec.builder(
                "${tag.name.humanize().capitalize()}Content",
                it.component1().parameterizedBy(STAR)
...
private fun Generator.generateTagFunction(tag: TagInfo, bodyType: Pair<TypeName, MemberName>?): FunSpec.Builder {
    ...
    val bodyParamName = "body"
    bodyType?.component1()?.let {
        funBuilder.addParameter(ParameterSpec(name = bodyParamName, type = it))
    }
    ...
    val bodyExpression = if (bodyType != null) {
        CodeBlock.of("%M(%N)", bodyType.component2(), bodyParamName)
...
private fun getAttributeValueStringFn(
    type: AttributeType,
    variable: String
): CodeBlock {
    if (type == AttributeType.String) return CodeBlock.of("%N", variable)
    val conversionCode = when (type) {
        is AttributeType.Enum -> CodeBlock.of("it()?.%N", "value")
        AttributeType.EventHandler -> throw UnsupportedOperationException("event handlers are not considered attributes")
        is AttributeType.NamedBoolean -> getNamedBooleanStringFn(type)
        AttributeType.Number, AttributeType.String,
        AttributeType.URI, AttributeType.Boolean -> CodeBlock.of("it()?.toString()")

        AttributeType.Toggle -> CodeBlock.of("if(it()?:false) \"true\" else null")
    }
...
private fun getNamedBooleanStringFn(type: AttributeType.NamedBoolean): CodeBlock = CodeBlock.of(
    "%N()?.let { if(it) %S else %S }",
    "it", type.trueName, type.falseName
)
...
private fun Generator.getAttributeParameter(
    ...
    val attributeType = getAttributeType(attribute.name, qualifiedName, attribute.type)
        .copy(nullable = !attribute.required)
    val paramType = attributeType as? LambdaTypeName ?: LambdaTypeName.get(returnType = attributeType)
        .copy(nullable = !attribute.required)
...
private fun Generator.getAttributeType(name: String, qualifiedName: String, type: AttributeType): TypeName {
    return when (type) {
        ...
        is AttributeType.Enum -> getAttributeEnum(name, qualifiedName, type).typeName()
    }
}
...
private data class TagGroupFile(
...
private fun Generator.generateTagGroup(
    group: TagGroup,
    ...
    val renderFn = generateRenderFunction(incompleteSpec, incompleteImpl, packageName, fileName)
    ...
    val completeFile = run {
        ...
        val completeImpl =
            implementation.build().asFile(packageName ?: "tags.content", fileName ?: completeSpec.fileName)
        TagGroupFile(
...
private fun Generator.generateRenderFunction(
    contentType: FileContent.Type,
    contentImpl: FileContent.Type,
    packageName: String?,
    fileName: String?
): FileContent.Function = FunSpec.builder("render")
    .addModifiers(KModifier.INLINE)
    .addParameter(
        ParameterSpec(
            "body", LambdaTypeName.get(
                receiver = contentType.typeName().parameterizedBy(STAR),
                returnType = UNIT
            )
        )
    )
...
/**
 * Generates the interface of the "Body-receiver" the type that is given to the body-lambda and as component2 the reference to the function for that type
 */
private fun Generator.getContentType(
    content: TagContent,
    fallbackName: String,
    filename: String? = null
): Pair<ClassName, MemberName>? {
...
private fun <K, V> MutableMap<K, V>.nonConcurrentComputeIfAbsent(key: K, defaultValue: () -> V): V {
